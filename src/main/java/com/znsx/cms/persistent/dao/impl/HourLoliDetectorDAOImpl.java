...
/**
 * HourLoliDetectorDAOImpl
 * 
 * @author huangbuji
 *         <p />
 *         Create at 2015-9-9 上午9:39:07
 */
public class HourLoliDetectorDAOImpl extends
		BaseDAOImpl<HourLoliDetector, String> implements HourLoliDetectorDAO {
	@Override
	public HourLoliDetector getLastRecord() {
		Session session = getSession();
		Criteria criteria = session.createCriteria(HourLoliDetector.class);
		criteria.setFirstResult(0);
		criteria.setMaxResults(1);
		criteria.addOrder(Order.desc("dateTime"));
		return (HourLoliDetector) criteria.uniqueResult();
	}

	@Override
	public void batchInsert(List<HourLoliDetector> list) {
		Session session = getSession();
		batchInsert(list, session);
	}

	@Override
	public List<HourLoliDetector> list(String sn, Long beginTime, Long endTime,
			int start, int limit) {
