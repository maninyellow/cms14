...
/**
 * MonthLoliDetectorDAOImpl
 * 
 * @author huangbuji
 *         <p />
 *         Create at 2015-9-9 上午9:46:00
 */
public class MonthLoliDetectorDAOImpl extends
		BaseDAOImpl<MonthLoliDetector, String> implements MonthLoliDetectorDAO {
	@Override
	public MonthLoliDetector getLastRecord() {
		Session session = getSession();
		Criteria criteria = session.createCriteria(MonthLoliDetector.class);
		criteria.setFirstResult(0);
		criteria.setMaxResults(1);
		criteria.addOrder(Order.desc("dateTime"));
		return (MonthLoliDetector) criteria.uniqueResult();
	}

	@Override
	public void batchInsert(List<MonthLoliDetector> list) {
		Session session = getSession();
		batchInsert(list, session);
	}

	@Override
	public List<MonthLoliDetector> list(String sn, Long beginTime,
